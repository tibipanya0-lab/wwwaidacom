...
import { ArrowLeft, Flame, ArrowDownWideNarrow, TrendingDown, Search, ExternalLink, ShoppingBag, Tag, Copy, Check, Loader2, Star } from "lucide-react";
...
const Deals = () => {
  ...
  // Fetch products from DB instead of external API
  ...
  // Fetch coupons from DB to match with products
  const { data: coupons = [] } = useQuery({
    queryKey: ["deals-coupons"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("coupons")
        .select("*")
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      <SEOHead
        ...
        description={{
          hu: "Fedezd fel a legjobb akciókat és leárazásokat! A legnépszerűbb termékek a legjobb árakkal.",
          en: "Discover the best deals and discounts! Popular products at the best prices.",
          uk: "Відкрийте найкращі акції та знижки! Популярні товари за найкращими цінами.",
        }}
        canonical="/akciok"
      />
      <CityScene3D />
      ...
      <main className="flex-1 overflow-y-auto pb-20">
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="mb-6 text-center">
            ...
            <p className="text-muted-foreground text-sm">Válogatott termékek a legjobb árakkal · Értékelés és népszerűség alapján</p>
          </div>
          ...
          <div className="mb-6 flex flex-col gap-4">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              {sorted.length} termék az adatbázisból
            </p>
          </div>
          ...
          {!isLoading && sorted.length > 0 && (
            <div className="flex flex-col gap-3">
              {sorted.map((product) => {
                ...
                const starRating = product.rating && product.rating > 5 ? product.rating / 20 : product.rating;
                  ...
                  >
                    ...
                    <div className="flex flex-1 flex-col p-4 gap-3 min-w-0">
                      ...
                      <a href={product.affiliate_url || "#"} target="_blank" rel="noopener noreferrer nofollow" className="hover:underline">
                        <h3 className="font-semibold text-foreground text-sm sm:text-base leading-snug line-clamp-2">{product.title}</h3>
                      </a>
                      ...
                      <div className="flex items-baseline gap-2 flex-wrap">
                        ...
                        {product.review_count != null && product.review_count > 0 && (
                          <span className="text-xs text-muted-foreground">· {product.review_count.toLocaleString("hu-HU")} vélemény</span>
                        )}
                        {starRating != null && starRating > 0 && (
                          <span className="text-xs text-muted-foreground flex items-center gap-0.5">
                            · <Star className="h-3 w-3 fill-amber-400 text-amber-400" /> {starRating.toFixed(1)}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <div className="flex items-center gap-2 rounded-lg border border-border/50 px-3 py-2 sm:flex-1">
                          ...
                          <span className="text-xs sm:text-sm text-muted-foreground">Várható szállítás: {shippingText}</span>
                        </div>
                        ...
                        {coupon ? (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <code className="text-sm font-mono font-bold text-foreground truncate block">{coupon.code}</code>
                            </div>
